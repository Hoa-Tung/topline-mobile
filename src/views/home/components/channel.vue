...
export default {
  ...
  computed: {
    ...
    // 计算属性其实也拥有watch的功能 但他的作用是用于当数据改变之后重更新计算返回一些数据供我们使用
    recommendChannels () {
  ...
  methods: {
    async loadAllChannels () {
      try {
        ...
        data.channels.forEach(item => {
          ...
          item.finished = false // 控制该频道上拉加载是否已经加载完毕
    ...
    async handleAddChannel (item) {
      // userChannels 是props数据
      // props数据有个原则：单项数据流
      // 数据只受父组件影响 但是反之不会
      // 但是引用类型除外 即便是这样：也最好不要利用这个特点
      // 建议做法就是讲数据传递给父组件，让组件自己去修改
      // this.userChannels.push(item)
      // 截取一个新的数组，操作这个数组，操作结束将结果传递给父组件，让父组件自己去修改
      // 始终记住一个原则：props数据是单向的，不要在子组件中修改她，始终有父组件去膝盖从而影响他
